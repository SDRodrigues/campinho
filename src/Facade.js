...
let spreadMines = (board, minesAmount) => {
    let rows = board.length
    let columns = board[0].length
    let minesPlanted = 0

    while (minesPlanted < minesAmount) {
        let rowSel = parseInt(Math.random() * rows, 10);
        let columnSel = parseInt(Math.random() * columns, 10);
...
let createMinedBoard = (rows, columns, minesAmount) => {
